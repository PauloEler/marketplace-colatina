...
HOME_CIDADE_VIVA_ENABLED = os.environ.get(
    "HOME_CIDADE_VIVA_ENABLED", "false"
).strip().lower() in {"1", "true", "on", "sim"}
HOME_2_ENABLED = os.environ.get("HOME_2_ENABLED", "false").strip().lower() in {
    "1",
    "true",
    "on",
    "sim",
...
HOME_FINISH_007B_ENABLED = os.environ.get(
    "HOME_FINISH_007B_ENABLED", "false"
).strip().lower() in {"1", "true", "on", "sim"}
...
try:
    ...
    HOME_CIDADE_VIVA_PRODUCT_LIMIT = 4
HOME_CIDADE_VIVA_PRODUCT_LIMIT = min(8, max(3, HOME_CIDADE_VIVA_PRODUCT_LIMIT))
...
def index():
    ...
    novidades_home = []
    metricas_cidade = []
    if home_cidade_viva_enabled:
        anuncios_home = anuncios[:HOME_CIDADE_VIVA_PRODUCT_LIMIT]
        novidades_home = anuncios[
            HOME_CIDADE_VIVA_PRODUCT_LIMIT : HOME_CIDADE_VIVA_PRODUCT_LIMIT + 3
        ]
        lojas_destaque_home = lojas_destaque[:3]
        pedidos_abertos = db.execute(
            "SELECT COUNT(*) FROM pedidos_servico WHERE status='aberto'"
        ).fetchone()[0]
        metricas_brutas = (
            ("anuncios", len(anuncios), "anúncios ativos", "Ver ofertas", "#ofertas"),
            (
                "empresas",
                len(lojas_linhas),
                "empresas com anúncios",
                "Ver empresas",
                "#home-stores-title",
            ),
            (
        ...
        metricas_cidade = [
            ...
            for identificador, valor, rotulo, acao, destino in metricas_brutas
            if valor > 0
        ]
    ...
    return render_template(
        "index.html",
        anuncios=anuncios,
        anuncios_home=anuncios_home,
        total_anuncios_home=len(anuncios),
        novidades_home=novidades_home,
        lojas_destaque=lojas_destaque_home,
        total_lojas_home=len(lojas_destaque),
